...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($nombre && $email && $password) {
        try {
            ...
            // 2. Rol por defecto = autor
            $idRol = $pdo->query(
                "SELECT id_rol FROM Rol WHERE nombre = 'autor'"
            )->fetchColumn();
            $pdo->prepare(
            ...
            // 3. Fila en Autor
            $pdo->prepare(
                "INSERT INTO Autor (id_usuario, afiliacion)
                 VALUES (?, 'Afiliación por defecto')"
            )->execute([$idUsuario]);

            $pdo->commit();
            header('Location: /gescon/PHP/auth/login.php?registro=ok');
            exit;
...
<div class="card shadow p-4" style="min-width: 380px;">
    ...
    <div class="text-center">
        ...
        <a href="/gescon/PHP/auth/login.php" class="btn btn-outline-secondary w-100">Iniciar sesión</a>
    </div>
</div>
</body>
</html>
